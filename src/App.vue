...
<template>
  ...
  <div v-else class="flex min-h-screen">
    <aside ref="mobileNavigation" class="fixed inset-y-0 left-0 z-20 flex w-[250px] flex-col border-r border-app-border bg-white px-4 pb-[18px] pt-[25px] transition-[width,transform,visibility] duration-[250ms] max-[920px]:invisible max-[920px]:pointer-events-none max-[920px]:-translate-x-full" :class="[{ 'visible pointer-events-auto translate-x-0 shadow-[15px_0_45px_rgba(36,33,31,.15)] max-[920px]:delay-0': mobileNavOpen }, sidebarCollapsed ? 'min-[921px]:w-[72px]' : 'min-[921px]:w-[250px]']" :role="mobileNavOpen ? 'dialog' : undefined" :aria-modal="mobileNavOpen ? 'true' : undefined" aria-label="Seitennavigation">
      ...
      <RouterLink class="relative mt-auto flex items-center gap-[10px] rounded-lg border-t border-[#ebe7e2] px-2 pb-0 pt-[15px] text-inherit no-underline hover:bg-[#f5f2ee]" :class="sidebarCollapsed ? 'min-[921px]:justify-center min-[921px]:px-0' : ''" to="/account" aria-label="Zu den Kontoeinstellungen" :title="sidebarCollapsed ? 'Kontoeinstellungen' : undefined" @click="closeMobileNavigation()">
        <div class="grid size-[38px] shrink-0 place-items-center rounded-full bg-[#dceae7] text-xs font-bold text-petrol">{{ accountInitials(store.account) }}</div>
        <div :class="sidebarCollapsed ? 'min-[921px]:hidden' : ''"><strong class="block text-[13px]">{{ store.account.firstName }} {{ store.account.lastName }}</strong><small class="mt-[2px] block text-[11px] text-app-muted">{{ store.account.role === 'root' ? 'Inhaber' : 'Mitarbeiter' }}</small></div>
      </RouterLink>
      <button class="relative mt-[10px] hidden w-full items-center justify-center gap-[6px] rounded-lg border-0 bg-transparent px-2 py-[9px] text-center text-[11px] font-bold text-app-muted hover:bg-[#f5f2ee] hover:text-app-text min-[921px]:flex" :title="sidebarCollapsed ? 'Navigation ausklappen' : 'Navigation einklappen'" @click="sidebarCollapsed = !sidebarCollapsed">
    ...
    <section class="w-full transition-[margin,width] duration-[250ms] min-[921px]:ml-[250px] min-[921px]:w-[calc(100%-250px)]" :class="sidebarCollapsed ? 'min-[921px]:ml-[72px] min-[921px]:w-[calc(100%-72px)]' : ''">
          ...
          <AppPanel >
            ...
            <div v-if="scheduleView === 'arrivals' && filteredArrivals.length">
              <article v-for="booking in filteredArrivals" :key="booking.id" class="grid grid-cols-[auto_minmax(155px,1fr)_110px_61px_max-content] items-center gap-[13px] border-b border-b-[#eeeae6] px-[22px] py-4 last:border-b-0 max-[680px]:grid-cols-[auto_1fr_auto] max-[680px]:p-[15px]">
                ...
                <div><strong class="block text-sm">{{ booking.customer.firstName }} {{ booking.customer.lastName }}</strong><span class="mt-[3px] block text-[11px] text-app-muted">{{ booking.pet.name }}</span></div>
                <div class="max-[680px]:col-start-2"><strong class="block text-[13px]">{{ booking.arrival }} Uhr</strong><span class="mt-[3px] block text-[11px] text-app-muted">{{ booking.room.name }}</span></div>
            ...
            <div v-else-if="scheduleView === 'departures' && filteredDepartures.length">
              <article v-for="departure in filteredDepartures" :key="departure.id" class="grid grid-cols-[auto_minmax(155px,1fr)_110px_auto] items-center gap-[13px] border-b border-b-[#eeeae6] px-[22px] py-4 last:border-b-0 max-[680px]:grid-cols-[auto_1fr_auto] max-[680px]:p-[15px]">
                ...
                <div><strong class="block text-sm">{{ departure.customer.firstName }} {{ departure.customer.lastName }}</strong><span class="mt-[3px] block text-[11px] text-app-muted">{{ departure.pet.name }}</span></div>
                <div class="max-[680px]:col-start-2"><strong class="block text-[13px]">Abreise heute</strong><span class="mt-[3px] block text-[11px] text-app-muted">{{ departure.room.name }}</span></div>
            ...
            <RouterLink class="flex items-center justify-end gap-[7px] border-t border-t-[#eeeae6] px-[22px] py-[13px] text-xs font-bold text-primary hover:underline" to="/check-in-out">Alle Check-ins und Check-outs <ArrowRight :size="16" /></RouterLink>
          </AppPanel>

          <AppPanel as="aside" >
            <header><div><h2><RouterLink class="text-inherit no-underline hover:text-primary hover:underline" to="/occupancy">Belegung ansehen</RouterLink></h2><p>Aktueller Stand</p></div><span class="text-[20px] font-bold [font-family:'Manrope',sans-serif] text-primary">{{ store.occupancyRate.value }} %</span></header>
            <div class="mx-[22px] mt-[23px] h-2 overflow-hidden rounded-full bg-[#eeeae6]"><span class="block h-full rounded-[inherit] bg-primary transition-[width] duration-[350ms]" :style="{ width: `${store.occupancyRate.value}%` }" /></div>
            <div class="px-[22px] py-[10px]">
              <div v-for="summary in store.occupancyByCategory.value" :key="summary.category" class="grid grid-cols-[auto_1fr_auto] items-center gap-[10px] border-b border-b-[#eeeae6] py-[15px]">
                ...
                <p class="m-0"><strong class="block text-xs">{{ summary.category }}</strong><small class="mt-[2px] block text-[10px] text-app-muted">{{ summary.occupied }} von {{ summary.capacity }} belegt</small></p><span class="mt-[2px] block text-[10px] text-app-muted">{{ summary.capacity }} Plätze</span>
              </div>
            </div>
          </AppPanel>
        </div>
      ...
      <AppContainer v-else-if="route.name !== 'intro'" class="grid min-h-[calc(100vh-70px)] place-items-center">
        <div class="w-full max-w-[520px] rounded-[14px] border border-app-border bg-white p-12 text-center">
          ...
          <p class="mb-[25px] mt-0 text-app-muted">{{ currentPage.description }}</p>
