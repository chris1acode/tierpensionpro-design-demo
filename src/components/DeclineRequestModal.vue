...
<template>
  <BaseModal labelled-by="decline-request-title" modal-class="decline-request-modal max-w-[470px]" @close="emit('close')">
    ...
    <form class="grid gap-2 mt-5" @submit.prevent="confirm">
      <label for="decline-request-reason" class="text-[11px] font-bold text-[var(--muted)]">Ablehnungsgrund</label>
      <textarea id="decline-request-reason" v-model="reason" rows="3" required placeholder="z. B. Zeitraum bereits ausgebucht" class="w-full resize-y rounded-lg border border-[var(--border)] bg-white px-[10px] py-[9px] text-[color:var(--text)] [font:inherit] focus:border-[var(--primary)] focus:outline focus:outline-2 focus:outline-offset-1 focus:outline-[#f2c3a7]"></textarea>
