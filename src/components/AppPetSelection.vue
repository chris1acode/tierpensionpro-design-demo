...
<template>
  <fieldset
    :disabled="disabled"
    class="m-0 min-w-0 rounded-lg border border-[var(--border)] px-[10px] pb-[9px] pt-0"
  >
    <legend class="px-[3px] text-[11px] font-bold text-[var(--muted)]">Tiere</legend>
    <p class="mb-[7px] mt-0 text-[11px] leading-[1.35] text-[var(--muted)]">{{ description }}</p>
    <label
      v-for="pet in pets"
      :key="pet.id"
      class="!mt-[5px] !flex !grid-cols-none items-center gap-[7px] !text-xs !font-semibold !text-[var(--text)]"
    >
      <input
