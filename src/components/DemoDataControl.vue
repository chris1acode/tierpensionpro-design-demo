...
<template>
  <aside class="relative z-40 ml-auto grid flex-none justify-items-end gap-[10px]" aria-label="Demodaten-Steuerung">
    <div v-if="open" ref="demoMenu" class="absolute right-0 top-[calc(100%+8px)] w-[264px] rounded-[13px] border border-[var(--border)] bg-white p-[17px] shadow-[0_14px_40px_rgba(36,33,31,0.18)]">
      <header class="grid grid-cols-[auto_1fr_auto] items-center gap-[10px]">
        ...
        <div><strong class="block">{{ store.demoEnvironment.label }}</strong><small class="mt-[2px] block text-[10px] text-[var(--muted)]">{{ store.demoEnvironment.scenario }}</small></div>
        <AppIconButton ref="closeButton" aria-label="Demodaten-Menü schließen" @click="closeMenu(true)"><X :size="20" /></AppIconButton>
      </header>
      <p class="my-4 text-sm leading-[1.6] text-[var(--text)]">Du arbeitest mit einem sicheren Beispieldatensatz. Änderungen betreffen keine echten Kundendaten.</p>
