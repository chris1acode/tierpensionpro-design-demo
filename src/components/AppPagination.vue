...
const buttonClasses =
  'grid h-8 min-w-8 place-items-center rounded-[7px] border border-app-border bg-white px-2 font-bold text-app-text enabled:hover:border-primary enabled:hover:bg-[#fff4ec] enabled:hover:text-primary disabled:cursor-not-allowed disabled:opacity-40'
const activeButtonClasses = 'border-primary bg-[#fff4ec] text-primary'
</script>

<template>
  <nav v-if="pageCount > 1" class="flex flex-wrap items-center justify-center gap-1.5 border-t border-app-border px-4 py-3.5" :aria-label="ariaLabel">
    ...
    <template v-if="pageItems">
      <template v-for="item in pageItems" :key="item">
        <span v-if="typeof item === 'string'" class="min-w-[12px] text-app-muted tracking-[.08em]" aria-hidden="true">…</span>
        <button
          v-else
          type="button"
    ...
    <span v-else class="min-w-[105px] text-center text-xs font-bold text-app-muted">Seite {{ currentPage }} von {{ pageCount }}</span>
