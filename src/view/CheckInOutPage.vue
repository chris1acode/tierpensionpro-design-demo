...
<template>
  <AppContainer class="operations-page">
    ...
    <AppPanel class="mt-[22px]">
      ...
      <div>
        <article v-for="event in pagedHistory" :key="event.id" class="grid grid-cols-[auto_minmax(150px,1fr)_minmax(170px,.7fr)_minmax(96px,120px)_auto] items-center gap-[14px] border-b border-b-[#eeeae6] px-[22px] py-[14px] last:border-b-0 max-sm:grid-cols-[auto_1fr] max-sm:px-4">
          ...
          <div class="min-h-7 max-sm:col-start-2 max-sm:justify-self-start"><AppButton v-if="event.type === 'check-in' && store.canUndoCheckIn(event.bookingId)" variant="text" class="text-[11px] text-primary-dark" type="button" @click="store.undoCheckIn(event.bookingId)"><RotateCcw :size="14" /> Rückgängig</AppButton></div>
